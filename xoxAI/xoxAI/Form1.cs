...
namespace xoxAI
{
    public partial class Form1 : Form
    {
        ...
        public void yapayZeka() //yapay zeka fonksiyonu
        {
            ...
            {
                Random random = new Random(); //random sınıfndan random adında nesne ürettik              
                for (int i = 0; i < 10; i++) //sayının dizi içerisinde daha öncesinde olma ihtimaline karşı 10 sefer çalışacak döngü oluşturuldu
                {
                    int sayi = random.Next(1, 10); //sayi adında rastgele 1'le 9 arasında sayı ürettik
                    if (sayiDizisi.Contains(sayi) == false) //üretilen sayı dizide var mı yok mu kontrol ettik, yoksa çalıştı varsa döngü tekrar sayı üretti, dizide olmayan sayıyı bulana kadar döngü çalışacak
                    {
                        yazdir(sayi); //sayiyi yazdır butonu ile çalıştırdı 
                        sayiDizisi.Add(sayi); //sayıyı tekrar seçilmemesi için diziye ekledi
                        break; //dizide olmayan sayıyı bulduğu için döngüyü kırdı
                    }                    
                }              
            }
        }
